...
const CartView = () => {
    ...
    const fetchCartItems = async () => {
      try {
        ...
        const total = data.cart_items.reduce(
          (acc, item) => acc + item.menu.harga * item.quantity,
          0
        );
  ...
  const handleChange = (e) => {
    ...
    if (name === "order_type") {
      updatedForm.payment_method =
        value === "delivery" || value === "takeaway" ? "dana" : "";
    }
  ...
  const isDeliveryOrTakeaway =
    form.order_type === "delivery" || form.order_type === "takeaway";
                  ...
                  <tr key={item.menu.id}>
                    ...
                    <td>
                      <button
                        ...
                        onClick={() => handleRemove(item.menu.id)}
                      >
                        Batalkan
                      </button>
                    </td>
            ...
            <div className="mb-3">
              ...
              {isDeliveryOrTakeaway && (
                <small className="text-danger">
                  Pembayaran hanya dapat dilakukan melalui Dana untuk {form.order_type}
                </small>
              )}
            </div>
